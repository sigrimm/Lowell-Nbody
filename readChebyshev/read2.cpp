...
int main(){
	...
	double timeStart = 8255.5;		//start time of integration
	double timeEnd = -11744.5;		//end time of integration
	double dt = -0.01;
	...
	int dts = (dt > 0.0) ? 1.0 : -1.0;	//sign of time step
...
//printf("GR a %d %.20g %.20g %.20g\n", i, ax[i], ay[i], az[i]);
			...
			for(int i = Nperturbers; i < N; ++i){
				...
				double REkm = RE / AUtokm;   //Earth radius in AU
...
//printf("dz %.20g %.20g %.20g\n", xE, yE, zE);
				...
				//double t1 = GM[2] * 3.0 * J2E * REkm * REkm / (2.0 * r5);
				double t1 = 3.0 * J2E * REkm * REkm / rsq / rsq / r / 2.0;
